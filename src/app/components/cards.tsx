...
const Cards = () => {
    const cards = [
        ...
        {
            title: {
                name: '<span class="text-tango-500">26% Increase</span> in B2B sales opportunity attribution using a multi-touch funnel', type: 'balance'
            },
            tags: ['Marketing', 'Branding', 'Web Design'],
    ...
    return (
        <div className="cards grid grid-cols-3 auto-rows-[384px] gap-4 px-48 mt-16">
            {cards.map(card => (
            <div className="group relative p-0.5 pb-3 bg-gradient-to-b from-darkslategrey-600/60 to-darkslategrey-600 rounded-xl cursor-pointer">
                <div className="info relative flex flex-col justify-between z-10 w-full h-58 px-6 py-6 text-left bg-neutral-100 rounded-t-[10px] overflow-hidden transition-all duration-500 group-hover:h-72">
                    ...
                    <div className={`tags relative flex ${card.tags.length === 3 ? 'justify-between' : 'space-x-4'} items-center z-10 opacity-0 transition-opacity duration-300 group-hover:opacity-100`}>
                        {card.tags.map(tag => (
                            <div className="tag px-4 py-0.5 bg-transparent border-[1px] border-darkslategrey-600 rounded-full text-darkslategrey-600 text-sm">{tag}</div>
                        ))}
                    </div>
